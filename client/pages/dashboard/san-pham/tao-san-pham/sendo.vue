			...
			<el-card :body-style="{ padding: '0px' }" shadow="hover">
					...
					<el-row :gutter="20">
						<el-col :span="12" :offset="0">
							<el-button type="text" @click="goBack" icon="el-icon-back" :loading="false" :plain="false" :round="false" :circle="false" :autofocus="false" size="medium" :disabled="false">
								<img style="margin-left: 15px;margin-bottom: -5px" :src="LoadImg('sendo')">
							</el-button>
						</el-col>
				...
				<div style="padding: 14px;">
						...
						<el-form-item label="Tên sản phẩm:" prop="name" align="left">
							<el-row :gutter="20">
								...
								<el-col :span="4" :offset="0">
									<el-input v-model="formData.sku" placeholder="SKU" size="medium" type="text" autocomplete="off"></el-input>
								</el-col>
							</el-row>
							<el-collapse>
					...
					<el-row v-show="attr(null,true).length === 0" :gutter="20">
						...
						<el-col :span="6" :offset="0">
							<el-form-item label="Đơn vị:" align="left">
								<el-select v-model="formData.unit_id" placeholder="Select" filterable>
									<el-option v-for="(data,index) in unit" :key="index" :label="data.label" :value="data.value" :disabled="false"></el-option>
								</el-select>
							</el-form-item>
					...
					<el-form-item  align="left">
						...
						<template v-if="attr('CheckBox').length > 0">
							<div v-for="(top,time) in Math.ceil((attr('CheckBox').length)/4)" :key="time">

								<el-row :gutter="20">
									<el-col v-for="(attribute,index) in attr('CheckBox').slice(time,time+4)" :key="index"  :span="6" :offset="0">

										<el-badge :hidden="!attribute.is_required" is-dot class="item">
											<el-select filterable multiple v-model="attribute.select" size="medium" :placeholder="attribute.name">
												<el-option v-for="(data,index) in attribute.attribute_values" :key="index" :label="data.value" :value="data" :disabled="false"></el-option>
											</el-select>
										</el-badge>
									</el-col>
								</el-row>
								<br v-if="attr('CheckBox').length > 4">
							</div>
							<el-divider direction="horizontal"></el-divider>
						</template>

						<template v-if="attr('ComboBox').length > 0">
							
							<div v-for="(top,time) in Math.ceil((attr('ComboBox').length)/4)" :key="time">
								<el-row :gutter="20">

									<el-col v-for="(attribute,index) in attr('ComboBox').slice(time,time+4)" :key="index" :span="6" :offset="0" >
										<el-badge :hidden="!attribute.is_required" is-dot class="item">
											<el-select filterable v-model="attribute.select" size="medium" :placeholder="attribute.name">
												<el-option v-for="(data,index) in attribute.attribute_values" :key="index" :label="data.value" :value="data" :disabled="false"></el-option>
											</el-select>
										</el-badge>
									</el-col>
								</el-row>
								<br v-if="attr('ComboBox').length > 4">
							</div>
							<el-divider direction="horizontal"></el-divider>

						</template>
						<template v-if="attr('TextBox').length > 0">
							
							<div v-for="(top,time) in Math.ceil((attr('TextBox').length)/4)" :key="time">

								<el-row :gutter="20">
									<el-col v-for="(attribute,index) in attr('TextBox').slice(time,time+4)" :key="index" :span="6" :offset="0">
								...
								<br v-if="attr('TextBox').length > 4">
							</div>
							<el-divider direction="horizontal"></el-divider>
						</template>
					</el-form-item>

					<el-form-item  label="Giá sản phẩm:" align="left">
					...
					<el-form-item align="center">
						<el-button @click="Submit" type="primary" :loading="loadingForm" size="small">Submit</el-button>
					</el-form-item>
				</template>
			</el-form>
...
<el-dialog title="Nhập URL sản phẩm trên SENDO" :visible.sync="copy_visible" :center="false" :fullscreen="false" top="25vh" width="45%">
	<el-alert
...
<br>
<el-input placeholder="Dán link sản phẩm tại đây ..." size="medium" v-model="copy_url" type="text" autocomplete="off"></el-input>
<br>
			...
			Result(){
				...
				return _.ShopCustom.map(e=>{
					...
					let data =  {
						...
						"stock_quantity":_.formData.stock,
						"description":_.formData.description,
						"cat_4_id":_.formData.category.slice(-1)[0],
						"tags":_.formData.name.slice(0,30),
						"seo_keyword":_.formData.name.slice(0,30),
						"seo_title":_.formData.name.slice(0,30),
						"seo_description":_.formData.description,
						"height":_.formData.height,
						"length":_.formData.length,
						"width":_.formData.width,
						"weight":_.formData.weight,
						"unit_id":_.formData.unit_id,
						attribute_is_custom:false,
						"attributes":_.attr().map(e=>{
							return {
								"attribute_id": e.id,
								"attribute_name": e.name,
								"attribute_is_custom": e.is_custom,
								"attribute_is_checkout": e.is_checkout,
								"attribute_values":e.select

							}
						}),
						"avatar":{
							"picture_url":_.formData.images[0]
						},
						variants:e.variants,
						"pictures":_.formData.images.map(picture_url=>({picture_url})),
						"is_config_variant":_.attr().length > 0,
						"voucher":{
							"product_type":1,
							"is_check_date":false,
							"start_date":"",
		...
		methods:{
			...
			async Submit(){
				for(let e of this.Result){
					try{
						e.data.extended_shipping_package = {
						...
						let {data} = await this.$axios.post('/api/product/up',{accountec_id:e.accountec_id,data:e.data,ec:'sendo'});
			...
			async copy_product(){
				...
				try{
					...
					this.SameValueInput(data.ten_san_pham)
					this.formData.sku = data.skuId;
					this.formData.description = data.mo_ta;
			...
			async SameValueInput(value){
				...
				return await this.GetSuggesstionCategory();
				
			},
			
			attr(type_attr,required = false){
				let _ = this;
				let list_attribute =  _.formData.attribute.filter(({control_type,is_required,is_checkout})=>(type_attr ? control_type === type_attr : true) && ((!_.attr_sub ||  required) ? (is_required || is_checkout) : true));
				if((type_attr === 'CheckBox')){
					list_attribute = list_attribute.map(e=>{
						e.attribute_values.map(i=>{
							i.is_custom = false;
							i.is_selected = true;
							return e;
						});
			...
			async lazyLoadAction(node, resolve) {
				...
				let {data} = await this.$axios.post('/api/category/get',{ec:'sendo',parent_id:level ? value : '2'});
			...
			async GetAttribute(){
				...
				let {data} = await this.$axios.post('/api/category/attribute-sendo',{category_id});
