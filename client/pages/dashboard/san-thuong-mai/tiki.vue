	...
	<div  style="border: 1px solid #e4e4e4;border-radius: 5px;background-color: white;">
		...
		<el-row :gutter="20">
			<el-col align="left" :span="12" :offset="0">
				<el-image style="width:80px;margin-left:20px" src="https://res.cloudinary.com/doo4xi4rh/image/upload/v1606290606/tiki_cmgu5j.png"></el-image>
	...
	export default {
		...
		methods:{
			...
			async Refresh(e){
				...
				let {data} = await this.$axios.post('/api/refreshec/tiki',{id:e.id});
				if(data.error){
			...
			async accounts(pagination = 1){
				let {data} = await this.$axios.post('/api/accountec/get',{page:pagination,ec:'tiki'});
				this.data = data;
				return;
			},
			async LoadNewAccount(){
				if(this.$route.query.code) await this.$axios.post('/api/accountec/tiki',{code:this.$route.query.code});
				return;
			},
			async removeAcc(obj){
				await this.$axios.post('/api/accountec/remove',{id:obj.id,ec:'tiki'});
